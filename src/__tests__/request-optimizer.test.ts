...
describe('RequestOptimizer', () => {
  ...
  beforeEach(async () => {
    ...
    mockZlib.gzip.mockImplementation(async (data: string) => {
      const compressed = Buffer.from(data.slice(0, Math.floor(data.length * 0.7))); // 30% compression
      return compressed;
    });
  ...
  describe('initialization', () => {
    ...
    it('should initialize with custom configuration', () => {
      ...
      const customOptimizer = new RequestOptimizer(connectionPool, customConfig);
  ...
  describe('request optimization', () => {
    it('should execute basic optimized request', async () => {
      const mockExecuteRequest = jest.spyOn(connectionPool, 'executeRequest')
    ...
    it('should handle compression that doesnt provide savings', async () => {
      ...
      const callArgs = (connectionPool.executeRequest as jest.Mock).mock.calls[0][0];
  ...
  describe('response caching', () => {
    it('should cache successful responses', async () => {
      ...
      jest.spyOn(connectionPool, 'executeRequest').mockResolvedValue(mockResponse);
  ...
  describe('request batching', () => {
    ...
    it('should execute batch immediately when max size reached', async () => {
      ...
      const promise1 = requestOptimizer.optimizeRequest('url', { data: '1' }, { batchKey });
      const promise2 = requestOptimizer.optimizeRequest('url', { data: '2' }, { batchKey }); // Should trigger immediate execution
    ...
    it('should handle batch execution errors gracefully', async () => {
      jest.spyOn(connectionPool, 'executeRequest').mockRejectedValue(new Error('Batch failed'));

      const batchKey = 'error-batch';
      
      const promise1 = requestOptimizer.optimizeRequest('url', { data: '1' }, { batchKey });
      const promise2 = requestOptimizer.optimizeRequest('url', { data: '2' }, { batchKey });
  ...
  describe('auto-optimization', () => {
    ...
    it('should decrease cache size for low hit rate', () => {
      ...
      requestOptimizer.updateConfig({ responseCacheSize: 2000 });
    ...
    it('should adjust compression threshold based on compression ratio', () => {
      ...
      const originalThreshold = requestOptimizer.getConfig().compressionThreshold;
    ...
    it('should adjust batch timeout based on usage', () => {
      ...
      (requestOptimizer as any).pendingBatches.size = 10; // Average of 1 per batch
  ...
  describe('cache management', () => {
    it('should clear all caches', () => {
      ...
      (requestOptimizer as any).responseCache.set('key1', { data: 'value1', timestamp: Date.now(), headers: {} });
  ...
  describe('error handling', () => {
    it('should handle request execution errors', async () => {
      jest.spyOn(connectionPool, 'executeRequest').mockRejectedValue(new Error('Network error'));
  ...
  describe('request ID generation', () => {
    it('should generate unique request IDs', () => {
      const generateRequestId = (requestOptimizer as any).generateRequestId.bind(requestOptimizer);
