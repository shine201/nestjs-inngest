...
describe('Application Integration Tests', () => {
    ...
    module = await Test.createTestingModule({
      imports: [
        InngestTestingModule.forIntegrationTest({
          useRealServices: false, // Use mock services for easier testing
          includeController: true,
          mockConfig: testConfig,
        }),
      ],
      providers: [TestAppService, TestModule],
      controllers: [],
    }).compile();
  ...
  describe('Module Initialization', () => {
    ...
    it('should register test functions', () => {
      ...
      const testFunction = functionRegistry.getFunction('test-integration-function');
      expect(testFunction).toBeDefined();
      expect(testFunction!.config.id).toBe('test-integration-function');
      expect(testFunction!.config.name).toBe('Test Integration Function');

      const complexFunction = functionRegistry.getFunction('test-complex-function');
      expect(complexFunction).toBeDefined();
      expect(complexFunction!.config.id).toBe('test-complex-function');

      const errorFunction = functionRegistry.getFunction('test-error-function');
      expect(errorFunction).toBeDefined();
      expect(errorFunction!.config.id).toBe('test-error-function');
    });
    ...
    it('should create proper function definitions for Inngest', () => {
      ...
      const testFunctionDef = definitions.find(def => def.id === 'test-integration-function');
      expect(testFunctionDef).toBeDefined();
      expect(testFunctionDef!.name).toBe('Test Integration Function');
    ...
    it('should configure signature verification correctly', () => {
      const status = signatureVerificationService.getVerificationStatus(testConfig.signingKey);
  ...
  describe('Webhook Endpoint Integration', () => {
    it('should handle PUT requests for function introspection', async () => {
      ...
      expect(functionIds).toContain('test-integration-function');
      expect(functionIds).toContain('test-complex-function');
      expect(functionIds).toContain('test-error-function');
    });

    it('should handle POST requests for function execution', async () => {
      const testEvent = InngestTestUtils.createTestEvent(
        'test.integration',
        { message: 'Integration test event', value: 42 }
      );
    ...
    it('should handle complex function execution with steps', async () => {
      const testEvent = InngestTestUtils.createTestEvent(
        'test.complex',
        { operation: 'complex-test', data: [1, 2, 3] }
      );
      ...
      expect(response.body.result.message).toBe('Complex integration test successful');
    ...
    it('should handle function execution errors gracefully', async () => {
      const testEvent = InngestTestUtils.createTestEvent(
        'test.error',
        { shouldError: true, message: 'Force error' }
      );
      ...
      expect(response.body.error.function_id).toBe('test-error-function');
    ...
    it('should handle non-existent function requests', async () => {
      ...
      expect(response.body.error.message).toContain('Function not found: non-existent-function');
  ...
  describe('Service Integration', () => {
    it('should integrate InngestService with event sending', async () => {
      ...
      // Note: In test mode, send may return void or mock result
    });
    ...
    it('should integrate ExecutionContextService with function execution', async () => {
      const functionMetadata = functionRegistry.getFunction('test-integration-function');
      expect(functionMetadata).toBeDefined();
      expect(functionMetadata!.config.id).toBe('test-integration-function');
  ...
  describe('Configuration Integration', () => {
    ...
    it('should configure services with proper dependencies', () => {
      ...
      const healthStatus = module.get<InngestController>(InngestController).getHealthStatus();
  ...
  describe('Error Handling Integration', () => {
    it('should handle timeout scenarios', async () => {
      ...
      const webhookRequest = InngestTestUtils.createTestWebhookRequest(
        'test-integration-function', // Use existing function
        testEvent
      );
