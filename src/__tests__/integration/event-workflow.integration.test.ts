...
class WorkflowTestService {
  ...
  async startUserOnboarding(event: any, { step }: any) {
  ...
  async handleUserVerification(event: any, { step }: any) {
  ...
  async handleWelcomeNotification(event: any, { step }: any) {
  ...
  async handlePaymentProcessed(event: any, { step }: any) {
  ...
  async handleInventoryUpdate(event: any, { step }: any) {
    ...
    await step.run("check-low-stock", async () => {
      const lowStockItems = changes.changes.filter(
        (item: any) => item.newQuantity < 10,
      );
  ...
  async trackAnalytics(event: any, { step }: any) {
  ...
  async triggerUserOnboarding(
    ...
    try {
      ...
      // In test environment, if external API is not available, return mock response
      return { ids: [`test_${Date.now()}_${userId}`] };
    }
  }
  ...
  async triggerOrderCreation(orderId: string, userId: string, items: any[]) {
    try {
      ...
      // In test environment, if external API is not available, return mock response
      return { ids: [`test_${Date.now()}_${orderId}`] };
    }
  }
  ...
  async triggerWorkflowStart(workflowId: string, userId: string, type: string) {
    try {
      ...
      // In test environment, if external API is not available, return mock response
      return { ids: [`test_${Date.now()}_${workflowId}`] };
    }
  }
}
...
describe("Event Workflow Integration Tests", () => {
    ...
    module = await Test.createTestingModule({
      imports: [
        InngestTestingModule.forSmartIntegrationTest({
          additionalProviders: [WorkflowTestService],
        }),
      ],
    }).compile();
  ...
  describe("User Onboarding Workflow", () => {
    ...
    it("should execute complete user onboarding workflow", async () => {
      ...
      const onboardingEvent = InngestTestUtils.createTestEvent(
      ...
      const onboardingFunction = functionRegistry.getFunction(
        "user-onboarding-start",
      );
      ...
      const onboardingResult =
        await executionContextService.executeFunction(onboardingContext);
      ...
      const verificationEvent = InngestTestUtils.createTestEvent(
      ...
      const verificationFunction = functionRegistry.getFunction(
        "user-verification-handler",
      );
      ...
      const verificationResult =
        await executionContextService.executeFunction(verificationContext);
      ...
      const notificationFunction = functionRegistry.getFunction(
        "welcome-notification-handler",
      );
      ...
      const notificationResult =
        await executionContextService.executeFunction(notificationContext);
  ...
  describe("Order Processing Workflow", () => {
    ...
    it("should execute complete order processing workflow", async () => {
      ...
      const orderResult =
        await executionContextService.executeFunction(orderContext);
      ...
      const paymentEvent = InngestTestUtils.createTestEvent(
        "payment.processed",
        {
          orderId,
      ...
      const paymentResult =
        await executionContextService.executeFunction(paymentContext);
      ...
      const inventoryEvent = InngestTestUtils.createTestEvent(
        "inventory.updated",
        {
          items: [
            { id: "item1", quantity: -2 },
      ...
      const inventoryFunction =
        functionRegistry.getFunction("inventory-handler");
      ...
      const inventoryResult =
        await executionContextService.executeFunction(inventoryContext);
  ...
  describe("Analytics Tracking Workflow", () => {
    it("should track analytics events from all workflows", async () => {
      const analyticsEvent = InngestTestUtils.createTestEvent(
        "analytics.event.tracked",
        {
      ...
      const analyticsFunction =
        functionRegistry.getFunction("analytics-tracker");
      ...
      const result =
        await executionContextService.executeFunction(analyticsContext);
  ...
  describe("Complex Multi-Step Workflows", () => {
    ...
    it("should handle concurrent workflows for different users", async () => {
      ...
      for (let i = 0; i < userCount; i++) {
        ...
        workflows.push(
          executionContextService.executeFunction(onboardingContext),
        );
      }
  ...
  describe("Event Sending and Triggering Performance", () => {
    ...
    it("should maintain function execution performance under load", async () => {
      ...
      for (let i = 0; i < concurrentFunctions; i++) {
        ...
        executions.push(
          executionContextService.executeFunction(analyticsContext),
        );
      }
