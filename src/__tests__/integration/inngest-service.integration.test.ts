...
type IntegrationEventTypes = EventTypes<{
...
class IntegrationTestService {
  ...
  async processBatch(event: IntegrationEventTypes['batch.test'], { step }: any) {
    ...
    await step.run('send-batch-events', async () => {
      const batchEvents = results.map((batchResult, index) =>
        InngestTestUtils.createTestEvent(
          'batch.processed',
          {
            batchId,
            batchIndex: index,
  ...
  async processDelayed(event: IntegrationEventTypes['delayed.test'], { step }: any) {
  ...
  async handleErrors(event: IntegrationEventTypes['error.test'], { step }: any) {
  ...
  async handleRetry(event: IntegrationEventTypes['retry.test'], { step }: any) {
  ...
  async executeComplexWorkflow(event: IntegrationEventTypes['complex.workflow'], { step }: any) {
  ...
  async sendBatchTestEvents(events: Array<{ name: string; data: any }>): Promise<any> {
...
describe('Inngest Service Integration Tests', () => {
  ...
  describe('Real Inngest Service Integration', () => {
    ...
    it('should register all integration test functions', () => {
      ...
      functionIds.forEach(id => {
        ...
        expect(functionMetadata.id).toBe(id);
      });
    });
    ...
    it('should respect batch size configuration', async () => {
      ...
      for (let i = 0; i < 150; i++) { // Exceeds maxBatchSize of 100
      ...
      const result = await testService.sendBatchTestEvents(largeEventBatch);
  ...
  describe('Function Execution with Real Services', () => {
    it('should execute integration test handler with real services', async () => {
      ...
      const functionMetadata = functionRegistry.getFunction('integration-test-handler');
    ...
    it('should execute complex workflow with real services', async () => {
      const workflowSteps = ['validate', 'transform', 'process', 'notify', 'cleanup'];
      const workflowData = {
    ...
    it('should handle delayed processing with real services', async () => {
      ...
      const functionMetadata = functionRegistry.getFunction('delayed-processor');
      const executionContext = await executionContextService.createExecutionContext(
        functionMetadata,
        testEvent,
  ...
  describe('Retry Scenarios with Real Services', () => {
    it('should simulate retry scenarios', async () => {
      ...
      const failureContext = await executionContextService.createExecutionContext(
        functionMetadata,
        failureEvent,
      ...
      const successContext = await executionContextService.createExecutionContext(
        functionMetadata,
        successEvent,
      ...
      const result = await executionContextService.executeFunction(successContext);
  ...
  describe('Performance with Real Services', () => {
    ...
    it('should handle high-volume event sending', async () => {
      ...
      const startTime = Date.now();
      const batchPromises = batches.map(batch => testService.sendBatchTestEvents(batch));
