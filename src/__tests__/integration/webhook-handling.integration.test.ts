...
import { InngestWebhookError, InngestRuntimeError, InngestTimeoutError } from '../../errors';
import { Request } from 'express';
import * as crypto from 'crypto';

// Event types for webhook testing
type WebhookEventTypes = EventTypes<{
...
class WebhookTestService {
  ...
  async handleSimpleWebhook(event: WebhookEventTypes['webhook.test.simple'], { step }: any) {
  ...
  async handleComplexWebhook(event: WebhookEventTypes['webhook.test.complex'], { step }: any) {
  ...
  async handleWebhookError(event: WebhookEventTypes['webhook.test.error'], { step }: any) {
  ...
  async handleWebhookTimeout(event: WebhookEventTypes['webhook.test.timeout'], { step }: any) {
  ...
  async handleWebhookRetry(event: WebhookEventTypes['webhook.test.retry'], { step }: any) {
  ...
  async handleWebhookSignature(event: WebhookEventTypes['webhook.test.signature'], { step }: any) {
  ...
  async handleWebhookBatch(event: WebhookEventTypes['webhook.test.batch'], { step }: any) {
    ...
    for (let i = 0; i < items.length; i += batchSize) {
      ...
      const batchResult = await step.run(`process-batch-${Math.floor(i / batchSize)}`, async () => {
        return {
          batchIndex: Math.floor(i / batchSize),
          items: batch.map(item => ({ ...item, processed: true })),
          processedAt: new Date().toISOString(),
        };
      });
...
describe('Webhook Handling Integration Tests', () => {
  ...
  beforeAll(async () => {
    testConfig = {
      ...
      env: 'test',
      isDev: false, // Test production-like webhook handling
      logger: true,
      timeout: 5000,
      maxBatchSize: 100,
      strict: true,
      ...
      development: {
        enabled: false,
        disableSignatureVerification: false, // Test real signature verification
      },
    };
  ...
  describe('Basic Webhook Functionality', () => {
    ...
    it('should handle simple webhook POST requests', async () => {
      ...
      expect(response.body.result.result.originalMessage).toBe('Simple webhook test');
    });
  ...
  describe('Webhook Error Handling', () => {
    it('should handle webhook processing errors gracefully', async () => {
      ...
      expect(response.body.error.message).toContain('Webhook processing failed');
    ...
    it('should handle webhook validation errors', async () => {
      ...
      expect(response.body.error.message).toContain('Webhook validation failed');
    });
    ...
    it('should handle non-existent function errors', async () => {
      ...
      expect(response.body.error.message).toContain('Function not found: non-existent-webhook-function');
    });
  ...
  describe('Webhook Retry Mechanisms', () => {
    it('should handle webhook retry scenarios', async () => {
      ...
      expect(failureResponse.body.error.message).toContain('Webhook retry attempt 1 failed');
  ...
  describe('Webhook Signature Verification', () => {
    beforeEach(() => {
      ...
      if (signatureVerificationService instanceof MockSignatureVerificationService) {
    ...
    it('should verify valid webhook signatures', async () => {
      ...
      const signedRequest = createSignedWebhookRequest(webhookRequest, testConfig.signingKey);
    ...
    it('should reject webhooks with expired timestamps', async () => {
      ...
      const expiredRequest = createSignedWebhookRequest(webhookRequest, testConfig.signingKey || '', oldTimestamp);
  ...
  describe('Webhook Performance and Load Testing', () => {
    it('should handle concurrent webhook requests', async () => {
      ...
      responses.forEach((response, index) => {
        ...
        expect(response.body.status).toBe('ok');
        expect(response.body.result.result.originalMessage).toBe(`Concurrent webhook test ${index}`);
      });
    });
  ...
  describe('Webhook Configuration and Health', () => {
    it('should provide accurate health status', () => {
      ...
      expect(healthStatus.signatureVerification.isValid).toBe(true);
    });
    ...
    it('should handle webhook configuration with proper settings', () => {
      ...
      expect(testConfig.strict).toBe(true);
      expect(testConfig.development.disableSignatureVerification).toBe(false);
    });
  });
