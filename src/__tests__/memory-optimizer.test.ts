...
// Mock WeakRef and FinalizationRegistry for testing
const mockWeakRefInstances = new Map();
(global as any).WeakRef = jest.fn().mockImplementation((target) => {
  const id = Math.random().toString();
  mockWeakRefInstances.set(id, target);
  return {
    deref: () => mockWeakRefInstances.get(id),
  };
});

(global as any).FinalizationRegistry = jest
  .fn()
...
describe("MemoryOptimizer", () => {
  ...
  describe("object pooling", () => {
    describe("event objects", () => {
      it("should acquire and release event objects", () => {
        ...
        expect(obj3).toStrictEqual({}); // Should get a clean object from pool
      });
  ...
  describe("memory pressure handling", () => {
    ...
    it("should handle critical memory usage", () => {
      const performEmergencyCleanupSpy = jest.spyOn(
        memoryOptimizer as any,
