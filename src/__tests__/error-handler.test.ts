import { ErrorHandler, ErrorSeverity, withRetry, HandleErrors } from '../utils/error-handler';
import {
  InngestError,
  InngestConfigError,
...
describe('ErrorHandler', () => {
  ...
  describe('classifyError', () => {
    it('should classify InngestConfigError correctly', () => {
      const error = new InngestConfigError('Config validation failed', 'signingKey');
    ...
    it('should classify InngestFunctionError correctly', () => {
      const error = new InngestFunctionError('Function registration failed', 'test-function');
      const classification = errorHandler.classifyError(error);
    ...
    it('should classify InngestEventError correctly', () => {
      const error = new InngestEventError('Event processing failed', 'user.created');
      const classification = errorHandler.classifyError(error);
    ...
    it('should classify InngestTimeoutError correctly', () => {
      const error = new InngestTimeoutError('Function timed out', 'test-function', 'run-123', 30000);
      const classification = errorHandler.classifyError(error);
  ...
  describe('getSeverity', () => {
    it('should return correct severity for errors', () => {
      ...
      expect(errorHandler.getSeverity(criticalError)).toBe(ErrorSeverity.CRITICAL);
...
describe('withRetry', () => {
  ...
  it('should retry on retryable errors', async () => {
    ...
    const result = await withRetry(operation, { maxRetries: 2, retryDelay: 10 });
...
describe('HandleErrors decorator', () => {
  ...
  it('should respect custom decorator options', async () => {
    ...
    await expect(service.testMethodWithCustomOptions()).rejects.toThrow('Test error');
