...
describe("OptimizedFunctionRegistry", () => {
  ...
  describe("initialization", () => {
    ...
    it("should discover functions on module init", async () => {
      const discoverFunctionsSpy = jest.spyOn(
        registry as any,
        "discoverFunctions",
      );
    ...
    it("should precompute indexes after discovery", async () => {
      const precomputeIndexesSpy = jest.spyOn(
        registry as any,
        "precomputeIndexes",
      );
    ...
    it("should log discovery time", async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringMatching(
          /Discovered \d+ Inngest function\(s\) in \d+\.\d+ms/,
        ),
      );
    });
  });
  ...
  describe("function registration", () => {
    it("should register a function with metadata", async () => {
      ...
      await registry.registerFunctionOptimized(
        target,
        "testMethod",
        mockFunctionConfig,
      );
    ...
    it("should handle duplicate function registration", async () => {
      ...
      await expect(
        registry.registerFunctionOptimized(
          target,
          "testMethod",
          mockFunctionConfig,
        ),
  ...
  describe("function lookup by criteria", () => {
    beforeEach(async () => {
      ...
      (registry as any).functionsByTrigger.clear();
      (registry as any).functionsByClass.clear();
    ...
    it("should get functions by event trigger", () => {
      ...
      expect(eventFunctions.some((f) => f.config.id === "event-function")).toBe(
        true,
      );
    });
  ...
  describe("function definition creation", () => {
    ...
    it("should handle function definition creation errors", () => {
      ...
      jest
        .spyOn(registry as any, "createInngestFunctionOptimized")
        .mockImplementation(() => {
  ...
  describe("bulk operations", () => {
    ...
    it("should bulk get functions", async () => {
      ...
      expect(functions.some((f) => f.config.id === "bulk-function-0")).toBe(
        true,
      );
      expect(functions.some((f) => f.config.id === "bulk-function-2")).toBe(
        true,
      );
    });
  ...
  describe("cache management", () => {
    ...
    it("should cleanup old cache entries", () => {
      ...
      // Set old timestamp for the first cache entry
      const firstKey = cacheKeys[0];
      const cachedEntry = cache.get(firstKey);
      if (cachedEntry) {
        cachedEntry.lastAccessed = Date.now() - 130000; // 130 seconds ago (more than CACHE_TTL * 2)
      }
  ...
  describe("error handling", () => {
    ...
    it("should handle memory optimizer failures gracefully", async () => {
      jest
        .spyOn(memoryOptimizer, "acquireMetadataObject")
        .mockImplementation(() => {
  ...
  describe("memory efficiency", () => {
    ...
    it("should optimize object storage", async () => {
      ...
      expect(memoryOptimizer.optimizeObject).toHaveBeenCalledWith(
        mockFunctionConfig,
      );
    });
  });
