...
export class MetadataProcessor {
  ...
  private static validateFunctionId(id: any): void {
    ...
    if (trimmedId.length < rules.minLength || trimmedId.length > rules.maxLength) {
  ...
  private static validateTriggers(triggers: any, functionId: string): void {
    if (!triggers || !Array.isArray(triggers) || triggers.length === 0) {
      throw new InngestFunctionError(ERROR_MESSAGES.INVALID_TRIGGERS, functionId);
    }
  ...
  private static validateEventTrigger(trigger: any, index: number): void {
    if (!trigger.event || typeof trigger.event !== "string") {
      throw new Error(`Event trigger at index ${index} must have a valid event name`);
    }

    const eventName = trigger.event.trim();
    const rules = this.VALIDATION_RULES.eventName;
    
    if (eventName.length < rules.minLength || eventName.length > rules.maxLength) {
      throw new Error(
    ...
    if (trigger.if && (typeof trigger.if !== "string" || trigger.if.trim() === "")) {
      throw new Error(`Event trigger condition at index ${index} must be a non-empty string`);
    }
  }
  ...
  private static validateCronTrigger(trigger: any, index: number): void {
    if (!trigger.cron || typeof trigger.cron !== "string") {
      throw new Error(`Cron trigger at index ${index} must have a valid cron expression`);
    }
    ...
    if (!this.CRON_PATTERN.test(cronExpression)) {
      throw new Error(`Invalid cron expression "${cronExpression}". Must have 5 or 6 parts.`);
    }
  }
   ...
   */
  private static validateConcurrency(concurrency: any, functionId: string): void {
  ...
  private static validateRateLimit(rateLimit: any, functionId: string): void {
    if (!rateLimit.limit || typeof rateLimit.limit !== "number" || rateLimit.limit < 1) {
      throw new InngestFunctionError(
  ...
  private static validateRetries(retries: any, functionId: string): void {
    ...
    if (typeof retries !== "number" || retries < rules.min || retries > rules.max) {
      throw new InngestFunctionError(
  ...
  private static validateTimeout(timeout: any, functionId: string): void {
    ...
    if (typeof timeout !== "number" || timeout < rules.min || timeout > rules.max) {
      throw new InngestFunctionError(
   ...
   */
  static normalizeFunctionConfig(config: InngestFunctionConfig): InngestFunctionConfig {
    const cacheKey = this.createCacheKey(config);
  ...
  static batchValidateConfigs(configs: InngestFunctionConfig[]): void {
    ...
    const validationPromises = configs.map(async (config) => {
      try {
        ...
        errors.push(`Function ${config.id}: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
   ...
   */
  static areConfigsEquivalent(config1: InngestFunctionConfig, config2: InngestFunctionConfig): boolean {
