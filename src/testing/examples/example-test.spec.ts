...
import { InngestTestingModule, InngestTestUtils, createInngestTestingModule } from '../inngest-testing.module';
import { MockInngestService, MockExecutionContextService, MockSignatureVerificationService } from '../mocks';
...
describe('InngestTestingModule Examples', () => {
  describe('Unit Testing with Mocks', () => {
    ...
    beforeEach(async () => {
      ...
      mockExecutionContext = module.get<MockExecutionContextService>(ExecutionContextService);
    });
    ...
    it('should handle function execution with mocks', async () => {
      ...
      // Execute function
      const result = await service.handleUserCreated(testEvent, context);
      ...
      expect(context.step.runMock).toHaveBeenCalledWith('process-user', expect.any(Function));
      expect(context.step.sendEventMock).toHaveBeenCalledWith('send-welcome-email', expect.objectContaining({
        name: 'email.send',
    ...
    it('should test error scenarios', async () => {
      ...
      const testEvent = InngestTestUtils.createTestEvent('user.created', { userId: '123' }, { user: { id: 'test-user' } });
  ...
  describe('Controller Testing', () => {
    ...
    it('should handle webhook requests', async () => {
      const testEvent = InngestTestUtils.createTestEvent('user.created', { userId: '123' }, { user: { id: 'test-user' } });
      const webhookRequest = InngestTestUtils.createTestWebhookRequest('test-function', testEvent);
      ...
      // Execute webhook
      await controller.handlePost(mockRequest as any, mockResponse, mockRequest.headers! as Record<string, string>, webhookRequest);
    ...
    it('should handle signature verification failure', async () => {
      ...
      const mockRequest = MockSignatureVerificationService.createMockRequestWithInvalidSignature(webhookRequest);
      const mockResponse = {
        status: jest.fn().mockReturnThis(),
        json: jest.fn(),
      } as any;
  ...
  describe('Integration Testing', () => {
    ...
    it('should work with real services in test environment', async () => {
      ...
      const executionContext = module.get<ExecutionContextService>(ExecutionContextService);
  ...
  describe('Testing Utilities', () => {
    ...
    it('should create webhook requests with proper structure', () => {
      const event = InngestTestUtils.createTestEvent('user.created', { userId: '123' });
      const webhookRequest = InngestTestUtils.createTestWebhookRequest('test-function', event);
    ...
    it('should create function metadata for testing', () => {
      const metadata = InngestTestUtils.createTestFunctionMetadata('test-func', {
        name: 'Test Function',
  ...
  describe('Advanced Mock Scenarios', () => {
    ...
    it('should test step execution with custom results', async () => {
      ...
      context.step.runMock.mockResolvedValue({ userId: '123', processed: true, custom: 'data' });

      const result = await service.handleUserCreated(testEvent, context);

      expect(result).toEqual({ userId: '123', processed: true, custom: 'data' });
    });
    ...
    it('should test step failures', async () => {
      ...
      context.step.runMock.mockRejectedValue(new Error('Step failed'));

      await expect(service.handleUserCreated(testEvent, context)).rejects.toThrow('Step failed');
    });
