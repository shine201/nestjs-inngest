...
import { UserEventRegistry, UserEventFactory } from "../events/user.events";
...
export class UserService {
  ...
  async sendWelcomeEmail(
    ...
    await step.run("send-verification-email", async () => {
      ...
      try {
        ...
        logger.warn(
          `Verification email failed for user ${userId}, but continuing:`,
          error
        );
  ...
  async handleEmailFailure(
    ...
    const retryResult = await step.run("retry-send-email", async () => {
      try {
        ...
        switch (emailType) {
          case "welcome":
            result = await this.emailService.sendWelcomeEmail(recipient, {
              name: user.name,
            });
            break;
          case "verification":
  ...
  async trackUserVerification(
    event: InngestEvent<UserEventRegistry["user.verified"]>,
    { step, logger }: any
  ) {
    const { userId, email, verificationMethod } = event.data;

    logger.info(
      `Tracking verification for user ${userId} via ${verificationMethod}`
    );
    ...
    await step.run("send-congratulations-email", async () => {
      try {
        ...
        logger.warn(
          `Failed to send congratulations email to ${userId}:`,
          error
        );
