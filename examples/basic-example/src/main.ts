...
async function bootstrap() {
  ...
  try {
    ...
    logger.log(`🔗 Inngest webhook endpoint: http://localhost:${port}/api/inngest`);
    
    // Log helpful endpoints
    logger.log('\n📋 Available endpoints:');
    logger.log(`   GET  http://localhost:${port}/users              - List all users`);
    logger.log(`   POST http://localhost:${port}/users              - Create a new user`);
    logger.log(`   GET  http://localhost:${port}/users/:id          - Get user by ID`);
    logger.log(`   POST http://localhost:${port}/users/verify       - Verify user email`);
    logger.log(`   GET  http://localhost:${port}/users/health/status - System health check`);
    logger.log(`   PUT  http://localhost:${port}/api/inngest        - Inngest function introspection`);
    logger.log(`   POST http://localhost:${port}/api/inngest        - Inngest webhook endpoint`);
    ...
    if (!inngestAppId || !hasSigningKey || !hasEventKey) {
      ...
      logger.warn('   Please ensure you have set the required environment variables:');
